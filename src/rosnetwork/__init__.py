...
from rosnode import ROSNodeException
import socket
import xmlrpclib
import time
import sys
...
class RosLink(RosEntity):
    def __init__(
            self, conn_mode, topic, direction, local_node, local_host,
            local_port, remote_node, remote_host, remote_port,
            deanonymize=True):
        RosEntity.__init__(self)
        self.conn_mode = conn_mode
        self.topic = topic
        self.direction = direction
        self.local_node = deanonymize_node(local_node) \
            if deanonymize else local_node
        self.local_host = local_host
        self.local_port = local_port
        self.remote_node = deanonymize_node(remote_node) if \
            deanonymize else remote_node
        ...
        meta = "%s,topic,%s,%s,%s" % (
            ...
            #remote_node.replace("http://", "").
            #replace("/", ",").replace(":", ",")
            self.remote_node.replace(
                "http://", "").replace(":", "/"))
        self.set_uid(uid, meta)
...
@output:  a tuple of two lists new, del;
    each tuple is a list of tuples (uid, meta)
"""
...
def diff_entity_lists(old, cur):
    ...
    #return (zip(add_entities,
    #['' for e in add_entities]),
    #zip(del_entities, ['' for e in del_entities]))
    return (
        zip(add_entities,
            [meta_map.get(uid, '') for uid in add_entities]),
        zip(del_entities, ['' for uid in del_entities]))
...
class RosNetwork(Process):
    ...
    def update(self):
        ...
        try:
            ...
            for n in self._graph.nn_nodes:
                ...
                # Sample:
                #https://gist.github.com/mani-monaj/81e74a64185a19fb3684
                for l in links:
                    try:
                        local_port, = self.__regex_local_port.search(
                            l[6]).groups()
                        remote_host, remote_port = \
                            self.__regex_remote_host_port.search(l[6]).groups()
                        # l[1] is
                        ...
                        pass
                        #rospy.logwarn(
                        #"The result of getBusInfo()
                        #does not include detailed bus information.")
                    except IndexError:
                        pass
                        #rospy.logwarn(
                        #"Format error while parsing getBusInfo()")
                # TODO: Critical Limiting Factor
                time.sleep(0.01)
            ...
            rospy.logwarn(
                "Socket.error when communicationg with master,\
                 will try again.")
            self.nodes = self.old_nodes[:]
            self.hosts = self.old_hosts[:]
            self.links = self.old_links[:]
    ...
    # All class variables are local in this process
    def run(self):
        ...
        try:
            ...
            while not self._terminate_event.is_set():
                ...
                try:
                    ...
                    rospy.logwarn(
                        "Default interval for rosgraphmonitor is too small\
                         (%s). Last loop: %s" % (self._default_interval, diff))
                self._last_loop_time = time.time()
            rospy.loginfo("Rosgraph monitor exited cleanly.")
            return True
        except:
            rospy.loginfo(
                "rosgraphmonitor exited with '%s' : %s" %
                (sys.exc_info()[0], sys.exc_info()[1]))
            traceback.print_exc()
            #rospy.loginfo("Call Stack:")
